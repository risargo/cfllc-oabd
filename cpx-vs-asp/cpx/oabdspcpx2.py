...
def oabdspcpx_solve(data,verbose=False):
    print("\n solving oabd cplex subproblem solved also via cplex")
...
def oabd_add_bd_cuts(data,mod,i,_x,_z,context=None,is_callback=False):    
    def solve_cpx_subproblem(data,mod,i,_x,_z):
        global total_time_cut_separation
        start_time = time()
        J = range(data.nj)
        cpx = cplex.Cplex()
        ...
        beta  = cpx.variables.add(obj = [( _z )], lb = [0.0], ub=[cplex.infinity], types = ['C'], names=['beta'])[0]
        mu = cpx.variables.add(obj = [-float(data.gamma[i])], lb = [0.0], ub=[cplex.infinity], types = ['C'], names=['u'])[0]
        v = list(cpx.variables.add(obj = [-_x[j] for j in J], lb = [0.0] * data.nj, ub=[cplex.infinity] * data.nj, types = ['C'] * data.nj, names=["v(%d)" % j for j in J])) 
        ...
        if cpx.solution.get_status() == cpx.solution.status.unbounded:
            # get the ray (returns a list of values for variables)
            ray = cpx.solution.advanced.get_ray()

            cutinds[0] = mod._z[i]
            _beta,_mu = ray[beta],ray[mu]
            cutcoeffs[0] = -1.0 * _beta            
            for j in J:
                cutinds[j + 1] = j
                cutcoeffs[j + 1] = ray[v[j]] 
            rhs =  data.gamma[i] * _mu 
    ...
    global cutcoeffs
    global cutinds
    is_cut,inds,coeffs,rhs = solve_cpx_subproblem(data,mod,i,_x,_z)
    if is_cut == True:
